...
class Jumping {
    ...
    string ableToGet(int x, int y, vector<int> jumpLengths) {
        double longest = *max_element(jumpLengths.cbegin(), jumpLengths.cend());
        double dist = sqrt(x * x + y * y);
        double sum = accumulate(jumpLengths.cbegin(), jumpLengths.cend(), 0.0);
        sum -= longest;
        if (longest + sum >= dist && longest - sum <= dist) {
            return "Able";
        } else {
            return "Not able";
        }
