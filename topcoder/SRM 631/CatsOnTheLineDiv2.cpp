...
class CatsOnTheLineDiv2 {
    ...
    string getAnswer(vector<int> position, vector<int> count, int time) {
        ...
        REP(i, (int)position.size()){
            data.PB(MP(position[i], count[i]));
        }
        sort(ALL(data));
        REP(i, (int)data.size()){
            leftstart = max(leftstart, data[i].first - time);
            if (leftstart + data[i].second - 1 <= data[i].first + time){
                leftstart += data[i].second;
            }
            else{
                return "Impossible";
            }
