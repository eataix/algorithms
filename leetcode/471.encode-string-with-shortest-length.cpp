...
public:
  string encode(string s) {
    ...
    for (int len = 1; len <= N; ++len) {
      for (int left = 0, right; (right = left + len - 1) < N; ++left) {
        dp[left][right] = s.substr(left, len);

        for (int k = left; k < right; ++k) {
          auto leftStr = dp[left][k];
          auto rightStr = dp[k + 1][right];

          if (leftStr.size() + rightStr.size() < dp[left][right].size()) {
            dp[left][right] = leftStr + rightStr;
          }
        }

        auto t = s.substr(left, len);
        string replace = "";
        ...
        if (pos >= t.size()) {
          ...
          replace =
              to_string(t.size() / pos) + '[' + dp[left][left + pos - 1] + ']';
        }

        if (replace.size() < dp[left][right].size()) {
          dp[left][right] = replace;
        }
      }
    }
