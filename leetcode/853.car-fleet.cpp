...
public:
  int carFleet(int target, vector<int> &position, vector<int> &speed) {
    ...
    for (int i = 0; i < position.size(); ++i) {
      m[position[i]] = static_cast<double>(target - position[i]) / speed[i];
    }

    int res = 0;
    double curr = 0;

    vector<int> sorted_positions{position.begin(), position.end()};
    sort(sorted_positions.begin(), sorted_positions.end(), greater<int>());
