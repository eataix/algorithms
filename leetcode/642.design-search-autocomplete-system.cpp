...
class AutocompleteSystem {
  ...
  void dfs(TrieNode *root, vector<TrieNode *> &res) {
    if (root == nullptr) {
      return;
    }
    ...
    for (auto const &kv : root->child) {
      dfs(kv.second, res);
    }
  }

  vector<TrieNode *> search() {
    vector<TrieNode *> res;
    dfs(currNode, res);
    return res;
  }
...
public:
  ...
  vector<string> input(char c) {
    ...
    if (currNode != nullptr && currNode->child.count(c)) {
      currNode = currNode->child[c];
    } else {
      currNode = nullptr; // curr node is null so empty result for any further
                          // characters in current input
      return {};
    }

    auto matches = search();
    sort(matches.begin(), matches.end(),
         [](const TrieNode *t1, const TrieNode *t2) {
           return t1->count > t2->count ||
                  (t1->count == t2->count && t1->word < t2->word);
         });
    matches.resize(3);
    vector<string> res;
...
#ifdef DEBUG
int main() {
  AutocompleteSystem as{{"i love you", "island", "iroman", "i love leetcode"},
                        {5, 3, 2, 2}};
  as.input('i');
  as.input(' ');
  as.input('a');
