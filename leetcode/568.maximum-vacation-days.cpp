 ...
 * day. (Although you start at city 0, we could also fly to and start at other
 ...
 * stay at city 0 for the whole 3 weeks. For each week, you only have one day
 * to play and six days to work. So the maximum number of vacation days is
 * 3.
 *
 *
 *
...
public:
  int maxVacationDays(vector<vector<int>> &flights, vector<vector<int>> &days) {
    if (flights.empty() || flights[0].empty() || days.empty() ||
        days[0].empty()) {
      return 0;
    }
    vector<vector<int>> dp(days.size(), vector<int>(days[0].size() + 1));
      ...
      for (int cur_city = 0; cur_city < days.size(); cur_city++) {
        ...
        for (int dest_city = 0; dest_city < days.size(); dest_city++) {
          if (flights[cur_city][dest_city] == 1) {
            dp[cur_city][week] =
                max(days[dest_city][week] + dp[dest_city][week + 1],
                    dp[cur_city][week]);
          }
        }
      }
