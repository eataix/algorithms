...
public:
  ...
  bool book(int start, int end) {
    for (auto const &overlapping : overlappings) {
      if (start < overlapping.second && overlapping.first < end) {
        return false;
      }
    }

    for (auto const &booking : bookings) {
      if (start < booking.second && booking.first < end) {
        overlappings.push_back(
            {max(booking.first, start), min(booking.second, end)});
      }
    }
