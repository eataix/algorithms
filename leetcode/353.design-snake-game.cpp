...
public:
  /** Initialize your data structure here.
     ...
     second is at [1,0]. */
  SnakeGame(int width, int height, const vector<pair<int, int>> &food) {
    this->food = food;
    w = width;
    h = height;
    pos = 0;
  ...
  int move(string direction) {
    int currRow = q.back().first;
    int currCol = q.back().second;
    ...
    if (currRow < 0 || currRow >= h || currCol < 0 || currCol >= w ||
        hist.count({currRow, currCol})) {
      return -1;
    }

    hist.insert({currRow, currCol});
    q.push_back({currRow, currCol});

    if (pos >= food.size()) {
      return q.size() - 1;
    }

    if (currRow == food[pos].first && currCol == food[pos].second) {
      pos += 1;
      q.push_front(d);
      hist.insert(d);
    }
