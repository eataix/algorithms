 ...
 */
class Solution {
  int find(unordered_map<int, int> &parents, int id) {
    if (parents[id] == id) {
      return id;
    } else {
      return parents[parents[id]] = find(parents, parents[id]);
    }
  }
...
public:
  vector<int> findRedundantConnection(vector<vector<int>> &edges) {
    ...
    for (auto const &edge : edges) {
      ...
      auto uId = find(parents, u);
      auto vId = find(parents, v);
      if (uId == vId) {
        return edge;
      } else {
        parents[uId] = parents[vId];
      }
    }
