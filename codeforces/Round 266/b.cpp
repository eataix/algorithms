/** Micro Mezz Macro Flation -- Overheated Economy ., Last Update: Sep. 17th 2014 **/ //{
...
namespace MT{
    ...
    struct matrix{
        ...
        void init(rec e = 0){RST(d); if(e) REP(i, nn) d[i][i] = e;}
        matrix(rec e = 0){init(e);}

        matrix operator *(const matrix &rhs) const{
            matrix res; //REP_3(i, j, k, n, n, n) res.d[i][j] += d[i][k] * rhs.d[k][j];
            REP_2(i, j, nn, nn){
                LL tmp = 0; REP(k, nn) tmp += (LL) d[i][k] * rhs.d[k][j];
                res.d[i][j] = tmp % MOD;
            }
            return res;
        }
        ...
        inline int res(){
            int res = 0; REP(i, nn) INC(res, d[0][i]);
            return res;
        }
    };
...
int main()
{
    ...
    cin >> n >> a >> b;
    n *= 6;
    long long best_area = 10e10, best_a = a, best_b = b;
    for (long long new_a = a; new_a <= a + 50000LL; ++new_a) {
        long long new_b = n / new_a;
        if (n % new_a != 0) {
            new_b += 1;
        }
        new_b = max(b, new_b);
        long long area = new_a * new_b;
        if (area < best_area) {
            best_area = area;
            best_a = new_a;
            best_b = new_b;
    ...
    for (long long new_b = b; new_b <= b + 50000LL; ++new_b) {
        long long new_a = n / new_b;
        if (n % new_b != 0) {
            new_a += 1;
        }
        new_a = max(a, new_a);
