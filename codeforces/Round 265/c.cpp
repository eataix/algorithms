...
bool dfs(int pos, bool flag)
{
    ...
    for (char c = flag ? str[pos] : 'a'; c < 'a' + p; ++c) {
        ret[pos] = c;
        if (pos >= 1 && ret[pos - 1] == ret[pos]) {
            continue;
        }
        if (pos >= 2 && ret[pos - 2] == ret[pos]) {
            continue;
        }
        if (dfs(pos + 1, flag && str[pos] == c)) {
            return true;
        }
    }
    return false;
...
int main()
{
    ...
    if (dfs(0, true)) {
        cout << string{ret.cbegin(), ret.cbegin() + n} << endl;
    } else {
        cout << "NO" << endl;
    }
}
