/** Micro Mezz Macro Flation -- Overheated Economy ., Last Update: Sep. 17th
 * 2014 **/ //{
...
template <class T> inline T &checkMin(T &a, const T b) {
  if (b < a)
    a = b;
  return a;
}
template <class T> inline T &checkMax(T &a, const T b) {
  if (a < b)
    a = b;
  return a;
}
template <class T> inline T &checkMin(T &a, T &b, const T x) {
  checkMin(a, x), checkMin(b, x);
  return a;
}
template <class T> inline T &checkMax(T &a, T &b, const T x) {
  checkMax(a, x), checkMax(b, x);
  return a;
}
template <class T, class C> inline T &checkMin(T &a, const T b, C c) {
  if (c(b, a))
    a = b;
  return a;
}
template <class T, class C> inline T &checkMax(T &a, const T b, C c) {
  if (c(a, b))
    a = b;
  return a;
}
...
/* æ¨¡æ•°ä¸¤å€åˆšå¥½è¶… int æ—¶ã€‚
   inline int sum(uint a, int b){a += b; a %= MOD;if (a < 0) a += MOD; return
   a;} inline void INC(int &a, int b){a = sum(a, b);}
   */
...
struct matrix {
  ...
  matrix operator*(const matrix &rhs) const {
    ...
    REP_2(i, j, nn, nn) {
      LL tmp = 0;
      REP(k, nn) tmp += (LL)d[i][k] * rhs.d[k][j];
      res.d[i][j] = tmp % MOD;
    }
    return res;
  }
...
int main() {
  ...
  while (!sticks.empty()) {
    ...
    for (; end != sticks.end(); ++end) {
      if (*end > 0) {
        break;
      }
    }
    sticks.erase(sticks.begin(), end);
  }
}
